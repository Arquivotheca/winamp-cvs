...
static __declspec(naked) void Convert_S32_S16_mono_shift8_Unaligned(int16_t *destination_buffer, const int32_t **source_buffer, size_t channels, size_t count_per_channel, unsigned int decimation_bits)
{
	/*  general structure 
	...
	__asm
	{
		...
		mov ecx, DWORD PTR 0[ecx] // ecx holds right
		...
		jl do1
loop24:
		movups xmm0, XMMWORD PTR 0[ecx+edi*4] // 4 samples of left [L0 L1 L2 L3]
		movups xmm1, XMMWORD PTR 16[ecx+edi*4]
		movups xmm2, XMMWORD PTR 32[ecx+edi*4]
		movups xmm3, XMMWORD PTR 48[ecx+edi*4] // 4 samples of right [R0 R1 R2 R3]
...
void x86_SSE2_nsmath_pcm_Convert_S32_S16_mono_decimate8(int16_t *destination, const int32_t **source, size_t channels, size_t count_per_channel, unsigned int decimation_bits)
{
	...
	{
		Convert_S32_S16_mono_shift8_Unaligned(destination, source, channels, count_per_channel, decimation_bits);
	}
