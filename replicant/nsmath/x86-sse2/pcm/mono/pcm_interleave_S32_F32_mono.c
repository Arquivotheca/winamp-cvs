...
static __declspec(naked) void Convert_S32_F32_mono_Unaligned(float *destination_buffer, const int32_t **source_buffer, size_t channels, size_t count_per_channel, float gain)
{
	/*  general structure 
...
loop12:
		...
		movdqu XMMWORD PTR 16[eax+edi*4], xmm1
		movdqu XMMWORD PTR 32[eax+edi*4], xmm2
		movdqu XMMWORD PTR 48[eax+edi*4], xmm3
		movdqu XMMWORD PTR 64[eax+edi*4], xmm4
		movdqu XMMWORD PTR 80[eax+edi*4], xmm5
...
loop1:
		...
		movd DWORD PTR[eax+edi*8], xmm0 // store lower 32 bits (1 float) to destination
...
void x86_SSE2_nsmath_pcm_Convert_S32_F32_mono(float *destination, const int32_t **source, size_t channels, size_t count_per_channel, float gain)
{
	...
	{
		Convert_S32_F32_mono_Unaligned(destination, source, channels, count_per_channel, gain);
	}
